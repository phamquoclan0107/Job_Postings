...
export default function CategoryPage() {
  ...
  const openCreate = () => { setForm({ name: '', type: '' }); setFormErr({}); setModal('create') }
  const openEdit   = (cat) => { setEditTarget(cat); setForm({ name: cat.name, type: cat.type }); setFormErr({}); setModal('edit') }
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      if (modal === 'create') { await categoryService.create(form); toast.success('Tạo danh mục thành công!') }
      else { await categoryService.update(editTarget.id, form); toast.success('Cập nhật thành công!') }
      closeModal(); refetch()
    } catch (err) {
  ...
  return (
    <div>
      <PageTitle
        ...
        action={<Button onClick={openCreate}><PlusIcon /> Tạo danh mục</Button>}
      />
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              ...
              <tbody>
                {filtered.map((cat) => (
                  <tr key={cat.id} className="hover:bg-bg-hover transition-colors duration-[120ms]">
                    <td className="px-4 py-3.5 border-b border-border align-middle">
                      <span className="text-text-mute font-mono text-xs">#{cat.id}</span>
                    </td>
                    <td className="px-4 py-3.5 border-b border-border align-middle font-medium">{cat.name}</td>
                    <td className="px-4 py-3.5 border-b border-border align-middle">
                      <Badge color={typeColor(cat.type)}>{getCategoryTypeLabel(cat.type)}</Badge>
                    </td>
                    <td className="px-4 py-3.5 border-b border-border align-middle text-xs text-text-mute">{formatDateTime(cat.createdAt)}</td>
                    <td className="px-4 py-3.5 border-b border-border align-middle">
                      <div className="flex gap-1.5">
                        <Button size="sm" variant="ghost" onClick={() => openEdit(cat)}><EditIcon /></Button>
                        <Button size="sm" variant="danger" onClick={() => setDeleteTarget(cat)}><TrashIcon /></Button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {/* Create/Edit Modal */}
      {modal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[1000]" onClick={closeModal}>
          <div className="bg-bg-card border border-border rounded-xl p-8 max-w-[440px] w-[90%]" onClick={(e) => e.stopPropagation()}>
            ...
            <FormField label="Tên danh mục" required error={formErr.name}>
              <Input value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))}
                placeholder="Ví dụ: Kỹ thuật phần mềm" error={formErr.name} autoFocus />
            </FormField>
            <FormField label="Loại danh mục" required error={formErr.type}>
              <Select value={form.type} onChange={(e) => setForm((p) => ({ ...p, type: e.target.value }))} error={formErr.type}>
            ...
            <div className="flex gap-2.5 justify-end mt-2">
              ...
              <Button onClick={handleSave} loading={saving}>{modal === 'create' ? 'Tạo' : 'Lưu'}</Button>
            </div>
          </div>
        </div>
      )}
